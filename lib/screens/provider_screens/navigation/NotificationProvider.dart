...
class NotificationProvider extends ChangeNotifier {
  ...
  Future<void> fetchNotifications(String token) async {
    ...
    try {
      final response = await http.get(
        Uri.parse('$base_url/bid/api/notifications/provider/list'),
        headers: {
  ...
  Future<bool> markAsRead(String token, int notificationId) async {
    try {
      final response = await http.put(
        Uri.parse(
          '$base_url/bid/api/notifications/provider/read/$notificationId',
        ),
        headers: {
          'Authorization': 'Bearer $token',
          'Content-Type': 'application/json',
  ...
  Future<bool> markAllAsRead(String token) async {
    try {
      final response = await http.put(
        Uri.parse('$base_url/bid/api/notifications/provider/read-all'),
        headers: {
          'Authorization': 'Bearer $token',
          'Content-Type': 'application/json',
        },
